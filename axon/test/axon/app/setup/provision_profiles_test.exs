defmodule Axon.App.Setup.ProvisionProfilesTest do
  use ExUnit.Case, async: false # ファイルシステム操作のため

  alias Axon.App.Setup.ProvisionProfiles

  setup do
    tmp_dir = Path.join(System.tmp_dir!(), "axon_provision_test_#{System.unique_integer([:positive])}")
  ...
  test "copies sample to user path if it does not exist", %{user_path: user_path} do
    ...
    assert {:ok, ^user_path} = ProvisionProfiles.ensure_present(user_path: user_path, sample_path: sample_path)
  ...
  test "returns error if sample is missing", %{user_path: user_path} do
    ...
    assert {:error, {:provision_failed, ^user_path}} = ProvisionProfiles.ensure_present(user_path: user_path, sample_path: sample_path)
  end
end
