defmodule Axon.App.ConfigStoreTest do
  ...
  setup do
    ...
    on_exit(fn ->
      if original_path,
        do: System.put_env("AXON_PROFILES_PATH", original_path),
  ...
  defp write_tmp_profiles!(contents) do
    path =
      Path.join(System.tmp_dir!(), "config_store_test_#{System.unique_integer([:positive])}.yaml")
  ...
  test "initializes with config from file", %{name: name} do
    ...
    assert {:ok, %LoadConfig.Config{profiles: [%{name: "TestProfile"}]}} =
             ConfigStore.get_config(name)
  end
